    ...
    mObjectManager(0)
{
	...
	if (error)
	{
		BT_ERROR("MSGID_OBJECT_MANAGER_CREATION_FAILED", 0, "Failed to create object manager: %s", error->message);
		g_error_free(error);
		return;
	}
	...
	BT_DEBUG("Created object watch on bus %s for %s", busName.c_str(), path.c_str());
}
...
void ObjectWatch::handleInterfaceRemoved(GDBusObjectManager *manager, GDBusObject *object, GDBusInterface *interface, gpointer user_data)
{
	...
	BT_DEBUG("%s", __PRETTY_FUNCTION__);
